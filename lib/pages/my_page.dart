...
import 'package:twalk_app/widgets/pvp_list.dart';
...
class _MyPageState extends State<MyPage> {
  final cardMargin = const EdgeInsets.fromLTRB(15, 0, 15, 20);
                        ...
                        Container(
                          ...
                          child: ListView.builder(
                              ...
                              itemBuilder: (c, i) {
                                return Padding(
                                  padding:
                                      const EdgeInsets.fromLTRB(30.0, 0, 0, 0),
                                  child: Column(
                  ...
                  child: Column(
                    children: [
                      for (var i = 0; i < _jalkingHistory.length; i++)
                        PvpListEntity(
                          title: (target =
                              _jalkingHistory[i]["requester"]["id"] == 1
                                  ? _jalkingHistory[i]["receiver"]
                                  : _jalkingHistory[i]["requester"])["name"],
                          subtitle: Jiffy(DateTime.parse(
                                  _jalkingHistory[i]["createdAt"]))
                              .fromNow(),
                          rightText:
                              '${(distance = (Random().nextDouble() * 5)).toStringAsFixed(1)} km',
                          leftText: '${(distance * 50).toInt()}Kcal',
                          leftIcon: Icons.local_fire_department_outlined,
                          profileImage:
                              'assets/images/profile${target["id"] % 10 + 1}.png',
                          backgroundColor: common
                              .harmonyColors[i % common.harmonyColors.length],
                        )
                    ],
                  ),
                )
